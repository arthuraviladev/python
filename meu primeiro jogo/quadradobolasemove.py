...
from panda3d.core import CardMaker                   # Importa para criar formas geométricas simples (cards)
from panda3d.core import ClockObject                  # Importa para controle do tempo (delta time)
...
class MyApp(ShowBase):
    def __init__(self):
        ...
        cm = CardMaker("q")
        cm.setFrame(-0.1, 0.1, -0.1, 0.1)            # Define o tamanho do card (0.2x0.2 unidades)
        ...
        self.q1.setPos(-0.9, 0, 0)                    # Posição no eixo x: -0.9 (esquerda)
        ...
        self.q2.setPos(0.9, 0, 0)                     # Posição no eixo x: 0.9 (direita)
        ...
        self.accept("arrow_up", self.mover, [self.q1, 0, 0.05])    # Jogador 1 sobe (seta para cima)
        self.accept("arrow_down", self.mover, [self.q1, 0, -0.05]) # Jogador 1 desce (seta para baixo)
        self.accept("w", self.mover, [self.q2, 0, 0.05])           # Jogador 2 sobe (tecla W)
        self.accept("s", self.mover, [self.q2, 0, -0.05])          # Jogador 2 desce (tecla S)
